...
#
# RentinBerlin — Single-file Streamlit website with:
# - Premium, mobile-first Login / Signup / Reset (glassmorphism, adjustable transparency)
# - Background slideshow (assets/bg1.jpg bg2.jpg bg3.jpg) rotates every 15s (no rerun)
# - Email+Password auth (SQLite, PBKDF2-HMAC-SHA256)
...
# PBKDF2 iterations (reasonable for Streamlit demo; increase if CPU allows)
...
def safe_redirect_query_cleanup(keep_keys: List[str] = None):
    """
    Remove OAuth callback params like code/scope/state/error from the URL,
    but keep token `t` if it exists.
    """
    keep_keys = keep_keys or []
    to_remove = {"code", "scope", "state", "authuser", "prompt", "hd", "error", "error_description"}
    # new API
    try:
        ...
        for k, v in qp.items():
            if isinstance(v, list):
                qp_norm[k] = v[0] if v else ""
            else:
                qp_norm[k] = v
        for k in list(qp_norm.keys()):
            if k in to_remove and k not in keep_keys:
                qp_norm.pop(k, None)
        st.query_params.clear()
        for k, v in qp_norm.items():
            st.query_params[k] = v
        return
    except Exception:
        pass
...
def make_password_hash(password: str) -> str:
    ...
    return f"pbkdf2${PBKDF2_ITERS}${salt_b64}${hash_b64}"
...
def verify_password(password: str, stored: str) -> bool:
    try:
        ...
        if len(parts) == 3:
            # backward-compat: pbkdf2$<salt>$<hash> (assume PBKDF2_ITERS)
        ...
        calc = base64.b64encode(dk).decode("utf-8")
...
def clear_query_params_except_token():
    t = get_query_token()
    try:
        st.query_params.clear()
        if t:
    ...
    try:
        if t:
            st.experimental_set_query_params(t=t)
        else:
...
def google_oauth_handle_callback_if_present():
    ...
    try:
        ...
        # Clear only OAuth params (keep token if any)
        safe_redirect_query_cleanup(keep_keys=["t"])
...
def inject_global_css():
    st.markdown(
        ...
        <style>
          ...
          /* Make Streamlit inputs and buttons feel premium */
          .stTextInput input, .stTextInput textarea {
            border-radius: 14px !important;
            height: 46px !important;
          }
          .stTextInput input:focus, .stTextInput textarea:focus{
            box-shadow: 0 0 0 3px rgba(255,122,26,0.20) !important;
          }
...
def inject_auth_background_slideshow():
    """
    Fullscreen background slideshow with crossfade layers (JS interval).
    Uses assets/bg1.jpg bg2.jpg bg3.jpg if present; otherwise gradient.
    """
    bgs = []
        ...
        st.markdown(
            ...
            <style>
              ...
              .rb-bg::after {
                ...
                position:absolute;
                inset:0;
                background: linear-gradient(180deg, rgba(2,6,23,0.40) 0%, rgba(2,6,23,0.22) 45%, rgba(2,6,23,0.52) 100%);
              }
            </style>
            <div class="rb-bg"></div>
            """,
    ...
    components.html(
        ...
        <style>
          ...
          .rb-bg-wrap::after {{
            ...
            position:absolute;
            inset:0;
            background: linear-gradient(180deg, rgba(2,6,23,0.42) 0%, rgba(2,6,23,0.22) 45%, rgba(2,6,23,0.55) 100%);
          }}
        </style>

        <div class="rb-bg-wrap">
          <div class="rb-bg-layer active" id="rbL1"></div>
          <div class="rb-bg-layer" id="rbL2"></div>
        </div>

        <script>
          const imgs = {js_array};
          let idx = 0;
          const l1 = document.getElementById("rbL1");
          const l2 = document.getElementById("rbL2");
          l1.style.backgroundImage = `url('${{imgs[0]}}')`;
          l2.style.backgroundImage = `url('${{imgs[1 % imgs.length]}}')`;
          ...
          function step() {{
            ...
            if (topLayer === 1) {{
              l2.style.backgroundImage = `url('${{next}}')`;
              l2.classList.add("active");
              l1.classList.remove("active");
              topLayer = 2;
            }} else {{
              l1.style.backgroundImage = `url('${{next}}')`;
          ...
          setInterval(step, {int(BG_ROTATE_SECONDS * 1000)});
        </script>
        """,
        height=0
    )
...
# =============================
# AUTH UI CSS (5-star look, responsive, adjustable transparency)
# =============================
def inject_auth_css(card_alpha: float, card_blur_px: int):
    # clamp
    a = min(0.98, max(0.55, float(card_alpha)))
    blur = int(min(24, max(6, card_blur_px)))
    ...
    st.markdown(
        ...
        <style>
          ...
          .rb-auth-shell {{
            ...
            max-width: 460px;
            margin: 0 auto;
            padding: clamp(18px, 5vh, 48px) 0 clamp(18px, 6vh, 56px) 0;
          }}
          @media (max-width: 900px) {{
            .rb-auth-shell {{ max-width: 94vw; }}
          }}
          ...
          .rb-auth-tagline {{
            ...
            text-shadow: 0 12px 40px rgba(2,6,23,0.40);
          }}
          ...
          /* Glass card */
          .rb-auth-card {{
            background: rgba(255,255,255,var(--rb-card-alpha));
...
def star_svg() -> str:
    # inline SVG star (gold-ish via gradient; no external assets)
    return """
    <svg class="rb-star" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <defs>
      ...
      <path d="M12 2.5l2.95 6.15 6.8.98-4.92 4.8 1.16 6.78L12 18.95 5.99 21.2l1.16-6.78-4.92-4.8 6.8-.98L12 2.5z"
            fill="url(#g)"/>
    </svg>
    """
...
def auth_shell_open(tagline: str):
    # Adjustable transparency controls (works on phone/pc)
    if "ui_alpha" not in st.session_state:
        st.session_state["ui_alpha"] = 0.90
    if "ui_blur" not in st.session_state:
        st.session_state["ui_blur"] = 14

    with st.sidebar:
        st.markdown("### 🎨 UI Tuning")
        st.session_state["ui_alpha"] = st.slider(
            "Card transparency",
            min_value=0.55, max_value=0.98,
            value=float(st.session_state["ui_alpha"]),
            step=0.01,
            help="Higher = more solid card. Lower = more transparent glass."
        )
        st.session_state["ui_blur"] = st.slider(
            "Background blur",
            min_value=6, max_value=24,
            value=int(st.session_state["ui_blur"]),
            step=1,
            help="Glass blur strength."
        )
        st.caption("Tip: hide sidebar on mobile (top-left).")

    inject_auth_background_slideshow()
    inject_auth_css(card_alpha=st.session_state["ui_alpha"], card_blur_px=st.session_state["ui_blur"])
    ...
    if logo:
        ...
        logo_html = "<div class='rb-auth-logo'>🏠</div>"
    ...
    st.markdown(
        ...
        <div class="rb-auth-top">
          ...
          <div class="rb-stars">
            {stars}
            <span class="rb-rating-text">5.0 • trusted by renters</span>
          </div>
        </div>
        """,
        unsafe_allow_html=True
    ...
    if not google_oauth_is_configured():
        st.markdown(
            ...
            <div style="margin-top:8px; font-size:12px; color:#64748b;">
              Google OAuth not configured. Install <b>Authlib</b> and set Streamlit secrets.
            </div>
            """,
            unsafe_allow_html=True
        )
...
def page_login():
    ...
    st.markdown("<div class='rb-auth-sub'>Email & password, or sign in with Google.</div>", unsafe_allow_html=True)
    st.write("")
    ...
    st.markdown(
        """
        <div class='rb-foot'>
          By continuing, you agree to basic community rules and privacy.
        </div>
        """,
        unsafe_allow_html=True
    )
...
def page_app():
    ...
    with col1:
        ...
        st.write("")
        st.info("This is the base shell. Next: listings, posts, chat, saved homes, verification, etc.")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if tab == "profile":
        ...
        st.write("Add profile editing, avatar upload, posts, followers, verifications, etc.")
        st.markdown("</div>", unsafe_allow_html=True)
    elif tab == "settings":
        st.markdown("<div class='rb-card'>", unsafe_allow_html=True)
        st.write("## Settings (demo)")
        st.write("Add email verification, notifications, privacy, 2FA, etc.")
        st.markdown("</div>", unsafe_allow_html=True)
